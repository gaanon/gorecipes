...
INSERT INTO recipe_ingredients (recipe_id, ingredient_id, quantity_text, sort_order)
SELECT
    r.id as recipe_id,
    i.id as ingredient_id,
    recipe_data.quantity_text,
    recipe_data.sort_order
FROM recipes r, ingredients i,
(VALUES
    ('flour', '2 cups', 1),
    ('eggs', '2 large', 2),
    ('milk', '1.5 cups', 3),
    ('butter', '3 tbsp melted', 4),
    ('sugar', '2 tbsp', 5),
    ('salt', '1 tsp', 6),
    ('baking powder', '2 tsp', 7)
) AS recipe_data(ingredient_name, quantity_text, sort_order)
WHERE r.name = 'Classic Pancakes' AND i.name = recipe_data.ingredient_name
ON CONFLICT DO NOTHING;

-- For the Simple Pasta Marinara recipe
INSERT INTO recipe_ingredients (recipe_id, ingredient_id, quantity_text, sort_order)
SELECT
    r.id as recipe_id,
    i.id as ingredient_id,
...
FROM recipes r, ingredients i,
(VALUES
    ('tomatoes', '1 can (400g) crushed', 1),
    ('onions', '1 medium, diced', 2),
    ('garlic', '2 cloves, minced', 3),
    ('olive oil', '2 tbsp', 4),
    ('basil', '1/4 cup fresh, chopped', 5),
    ('pasta', '250g', 6),
    ('cheese', '50g Parmesan, grated', 7)
) AS recipe_data(ingredient_name, quantity_text, sort_order)
WHERE r.name = 'Simple Pasta Marinara' AND i.name = recipe_data.ingredient_name
ON CONFLICT DO NOTHING;
